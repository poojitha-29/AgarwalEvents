    ...
    <footer className="bg-maroon-900 text-beige-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          <div>
            ...
            <p className="font-lato text-sm text-beige-300 leading-relaxed mb-6">
              Leading <strong>wedding planner in Hyderabad</strong> and <strong>event management company in Hyderabad</strong>, specializing in luxury weddings, corporate events, birthday parties, and premium wedding decor across {LOCATIONS.join(', ')}.
            </p>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.to}>
                  <Link 
                    to={link.to} 
                    title={`Best wedding planner in Hyderabad - ${link.label}`}
                    className="font-lato text-sm text-beige-300 hover:text-gold-400 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
            ...
            <ul className="space-y-2">
              {ALL_SERVICES.slice(0, 7).map((s) => (
                <li key={s.slug}>
                  <Link 
                    to={`/services/${s.slug}`} 
                    title={`${s.name} in Hyderabad | Wedding & Event Planner`}
                    className="font-lato text-sm text-beige-300 hover:text-gold-400 transition-colors"
                  >
                    {s.name}
                  </Link>
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-start gap-3">
                ...
                {/* ✅ LOCAL SEO BOOST */}
                <span className="font-lato text-sm text-beige-300">
                  Wedding planner near me Hyderabad · {LOCATIONS.join(' · ')}
                </span>
      ...
      <div className="border-t border-maroon-800">
        <div className="container-custom py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="font-lato text-xs text-beige-400">
            © {new Date().getFullYear()} {SITE_NAME}. Best wedding planner in Hyderabad & event management company.
          </p>
          <p className="font-lato text-xs text-beige-400">
            Luxury Wedding Planning | Corporate Events | Birthday Parties in Hyderabad
          </p>
        </div>
      </div>
    </footer>
